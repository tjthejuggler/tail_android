...
**Last updated:** 2026-09-22T09:00Z
...
- **The bug (user report, 2026-09-22 morning).** The "⚖️ Compliance Over Time" chart showed games on 11/13/15 Sep (and others) as red **"no fresh test"** violations on days when the system itself had NOT applied any 24-hour penalty — and the "📜 Rating Since Readiness System" chart below showed the same days mostly green, i.e. the screen contradicted itself. Root cause: TWO authorization models. Enforcement had moved to the **rolling rated-play window** (15-minute REAL no-play gap; every CONTINUE_RATED audit and every rated game actually played re-anchors the clock — [`ChessRollingWindow.kt`](core-data/src/main/java/com/example/tail/data/ChessRollingWindow.kt:1)), but the charts still re-derived authorization with the RETIRED flat-60-minute rule evaluated at game END, and evaluated at game *start* elsewhere — so legitimate continuous sessions were mislabeled "no fresh test".
- **The fix — one verdict, everywhere.** Every game carries a **stored play-start `authorized` verdict** resolved by [`gameToRecord`](core-data/src/main/java/com/example/tail/data/ChessReadinessStatsCalculator.kt:214) with the SAME evidence the Chess Guard penalty detector uses (audit chain + rated-game spans — the logger in [`ChessReadinessLogStore.logGames`](app/src/main/java/com/example/tail/widget/ChessReadinessLogStore.kt:150) now passes both). Compliance series ([`computeComplianceSeries`](core-data/src/main/java/com/example/tail/data/ChessReadinessStatsCalculator.kt:533)), rating pools ([`computeRatingStats`](core-data/src/main/java/com/example/tail/data/ChessReadinessStatsCalculator.kt:625)), rating-history coloring, effectiveness categories ([`gameAuthorizationKind`](core-data/src/main/java/com/example/tail/data/ChessRollingWindow.kt:110)) and the freeplay comparison now all TRUST that stored verdict instead of replaying divergent rules; only the Denied-vs-Expired split is derived from the covering test's freshness (a stale Yellow/Red test is "bypass", not "defiance").
...
- **Honest history.** Unlike a pass, the readiness test you failed stays failed: the freeplay is recorded as a separate authorization with its own provenance flag ([`ChessReadinessEngine.ReadinessTest.freeplay`](core-data/src/main/java/com/example/tail/data/ChessReadinessEngine.kt:390)), mirrored into the detailed activity log; games authorized by it get `freeplayAtPlay: true` ([`ReadinessGameRecord`](core-data/src/main/java/com/example/tail/data/ChessReadinessStatsCalculator.kt:61)).
- **Stats comparison.** New "🎟 Freeplay vs Readiness Pass" section on the Chess Stats screen ([`ChessReadinessStatsScreen.kt`](app/src/main/java/com/example/tail/ui/ChessReadinessStatsScreen.kt:911)) splits authorized games by how play was authorized (computeFreeplayComparison in [`ChessReadinessStatsCalculator.kt`](core-data/src/main/java/com/example/tail/data/ChessReadinessStatsCalculator.kt:402)): games, wins, win rate, sessions, avg games/session and avg CCRS per source, plus the win-rate delta.
