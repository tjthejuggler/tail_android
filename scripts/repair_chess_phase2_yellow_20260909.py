#!/usr/bin/env python3
"""One-shot repair (2026-09-09): re-authorize rated play on the phone.

CONTEXT: the v3/v4 post-game audit pivoted a won blitz game (ΔE +0.571,
no blunders, ACPL 33) to drills with "chronic overload" because the ACWR
ratio sat at 1.31 — a hair over the 1.3 yellow bar — with the just-finished
game itself tipping it over (pre-game ratio 1.297). The 7-day acute window
is fully contained in the 28-day chronic window, so borderline ratios are
measurement noise.

ENGINE FIX (same commit): Rule 4 now (a) excludes the audited game from the
ACWR computation and (b) requires the ratio to clear the yellow/red bar by
a 0.05 boundary band. This script only clears the two stale PIVOT_TO_DRILLS
audit rows filed today so rated play is re-authorized while the green
readiness window is valid. Uses the SAFE transfer method (base64 over
`adb shell` + `run-as cp`).
"""
